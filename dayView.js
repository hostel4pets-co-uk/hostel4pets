...
async function loadDayView() {
    ...
    try {
        ...
        for (const ev of events) {
            const s = (stays[ev.petId] ||= {});

            const type = ev.type.toLowerCase().replace(/[\s-]+/g, "");

            const isIn = type.includes("checkin");
            const isOut = type.includes("checkout");
            ...
            if (isIn) {
                s.checkIn = normalise(new Date(ev.start));
                s.pet = ev;
            } else {
                s.checkOut = normalise(new Date(ev.end));
                s.pet = ev;
            }
        }
